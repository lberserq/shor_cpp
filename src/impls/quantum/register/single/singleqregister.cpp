...
SingleQRegister::SingleQRegister(int width, state_t startState, const mcomplex &amplitude, representation_t mode):
    m_width(width),
    m_representation(mode)
{
    ...
    m_states.resize(static_cast<state_t>(1ll << m_width));
...
const std::vector<mcomplex> & SingleQRegister::getStates() const{
    return m_states;
}
...
void SingleQRegister::allocSharedMem(int width) {
    ...
    m_states.resize(static_cast<state_t>(1 << m_width));
    ...
    for(state_t i = 0; i < m_sz; i++)
    {
        if (std::abs(m_states[i]) > g_eps) {
            ...
            if(state_id1 == flag)
            {
                ...
                for (int j = 0; j < id; j++) {
                    right_part |= static_cast<state_t>(1 << j);
                }
                right_part &= st;

                for (int j = sizeof(state_t) * 8 - 1; j > id; j--) {
                    left_part |= static_cast<state_t>(1 << j);
                }
                left_part &= st;
